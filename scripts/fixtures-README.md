...
`expected/` in every fixture is listed in that fixture's `.git/info/exclude` (written by the expectations script) so neither git nor the engine reports it as untracked. Alongside the JSON dumps, the v2 fixtures carry `expected/*.txt` — git's own output for `tag -l`, `stash list`, `reflog`, `ls-files -u`, `ls-files -v`, `check-ignore -v`, `log --graph`, `log -S`, `log --follow`, `blame --porcelain` (with and without `-w`), `rev-list --left-right --count`, `rev-list --bisect`, `rev-parse` and `shortlog -sn`. T10.5 added `log-head-date-order.txt` / `log-head-first-parent.txt` / `log-all-date-order.txt` (also for `crisscross`, `detached` and `perf-5k`), `commit-numstat.txt` (`git show --numstat --first-parent` per commit) and `branch-upstream.txt` / `branch-merged.txt` for the Branches table. `history/expected/preflight-outcome.txt` is the real `git rebase` result for `preflight/a` onto `main`, replayed on throwaway clones so the fixture itself is never rebased.
