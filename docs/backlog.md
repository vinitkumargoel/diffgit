...
| B10 | Settings toggle for the built-in excludes (`.DS_Store`, `._*`, `Thumbs.db`, `desktop.ini`). The engine accepts `builtinExcludes: false` (`IgnoreRules`), but the UI has no preference for it; the T8.1 amendment assumed one. | No UI setting exists yet; the default matches what most users want. | T8.1 |
