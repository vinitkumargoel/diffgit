...
| ~~B10~~ | ~~Settings toggle for the built-in excludes (`.DS_Store`, `._*`, `Thumbs.db`, `desktop.ini`).~~ **Closed in T11.4**: engine side landed in T10.4 (`OpenOptions.builtinExcludes` → `IgnoreRules`, `explainPath` naming `(built-in excludes)` as the source); the `Apply built-in excludes` checkbox now lives in the HelpDialog settings footer, writes the `builtinExcludes` pref and re-opens the session. | Done. | T8.1, engine T10.4, UI T11.4 |
