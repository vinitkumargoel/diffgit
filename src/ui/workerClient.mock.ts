/**
 * Recorded-JSON mock of the engine (T4.1), selected by `VITE_MOCK_ENGINE=1`. Serves
 * `src/test/recorded/{basic,worktree}.{repoinfo,diffresult}.json`, computes hunks from deterministic
 * texts, pushes stats over the sink in batches, and exposes `mutate()` so `probe()` signatures change.
 */
